...
import { LayoutGrid, Upload, Info, Bike, Car, Plane, Radio } from "lucide-react";
...
/** Map template path to a Lucide icon component. */
const TEMPLATE_ICONS: Record<string, typeof Bike> = {
  motorBike: Bike,
  raceCar: Car,
  smallPlane: Plane,
  fixedWingDrone: Radio,
};
      ...
      <div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6" role="group" aria-label="External aerodynamics templates">
              {aeroTemplates.map((tmpl) => {
                const Icon = TEMPLATE_ICONS[tmpl.path] ?? LayoutGrid;
                return (
                  <TemplateCard
                    key={tmpl.path}
                    tmpl={tmpl}
        ...
        {learningTemplates.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6" role="group" aria-label="Tutorial templates">
              {learningTemplates.map((tmpl) => (
                <TemplateCard
                  key={tmpl.path}
                  tmpl={tmpl}
                  Icon={LayoutGrid}
